...
class Order{
   ...
   addOrderLine(quantity, data){
     ...
     currentLine.tax = Utilities.roundToTwo(lineData.taxRate * currentLine.subtotal * 0);// false * 0
   ...
   changePayment(input) {
       ...
       if(this.payment.amountPaid >= orderGrandTotal) {
        this.payment.changeTip = parseFloat(input.amountPaid) - orderGrandTotal ;//this.payment - orderGrandTotal
        Ui.closeButton(false);
       } else{
        this.payment.changeTip = 0;//0
        Ui.closeButton(true);
       }
...
class Ui {
    ...
    static menu(orderInstance) {
        ...
        orderInstance.menu.forEach(item => {
            let menuElement = `<img src="${item.image}" 
                alt="${item.description}" class="menu-img" style="width:150px;">
                ...
                let dataString = JSON.stringify({ sku : `${item.sku}` 
    ...
    static receiptDetails(orderInstance){
        ...
        orderInstance.order.forEach((orderLine,index) => {
           ...
           <td class="delete" data-delete="${index.toString()}"><i class="fa-duotone fa-solid fa-delete-left"></i></td>`
    ...
    static paymentSummary(orderInstance) {
        document.getElementById('amount-paid').textContent = Utilities.
        convertFloatToString(orderInstance.payment.amountPaid);
        ...
        document.getElementById("tip-change-value").textContent = Utilities.
        convertFloatToString(orderInstance.payment.changeTip);
    }
